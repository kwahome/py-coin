...
class BlockchainStateTest(unittest.TestCase):
    def test_difficulty_calculation(self):
        ...
        with self.assertRaisesRegex(Exception, "The difficulty of the block does not match with the one supplied"):
            state.verify_and_apply_block(block)
    ...
    def test_undo(self):
        ...
        block = mine_block(
            previous, 16, BOB_ADDRESS, transactions, 120 * 16, state.calculate_difficulty(), time() + 150
        )
        state.verify_and_apply_block(block)
        previous = block.block_id
    ...
    def test_previous_validation(self):
        ...
        with self.assertRaisesRegex(Exception, "Invalid value for block previous field."):
            state.verify_and_apply_block(block)
    ...
    def test_reorg(self):
        ...
        for height in range(15):
            block = mine_block(
                previous, height, ALICE_ADDRESS, [], 120 * height, state.calculate_difficulty(), time() + 100
            )
            state.verify_and_apply_block(block)
            previous = block.block_id
        ...
        for height in range(8, 15):
            block = mine_block(
                previous, height, BOB_ADDRESS, [], 120 * height, state.longest_chain[height].difficulty, time() + 100
            )
            blocks.append(block)
            previous = block.block_id

        with self.assertRaisesRegex(
                Exception, "The total difficulty of the new state is less than or equal to the old state"
        ):
